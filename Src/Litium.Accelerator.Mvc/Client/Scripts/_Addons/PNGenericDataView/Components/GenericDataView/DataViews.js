...
import { GenericDataViewSettings, GenericDataViewPagination } from '.';
...
export const GenericDataView = ({
  ...
  const [
    ...
    setColumnsInsideContainerSmall,
  ] = useState(isInModal ? 1 : 1);
  const [
    columnsInsideContainerMedium,
    setColumnsInsideContainerMedium,
  ] = useState(isInModal ? 1 : 2);
  const [
    columnsInsideContainerLarge,
    setColumnsInsideContainerLarge,
  ] = useState(isInModal ? 1 : 3);

  const [columnsWithContainersSmall, setColumnsWithContainersSmall] = useState(
    isInModal ? 1 : 1
  );
  const [
    columnsWithContainersMedium,
    setColumnsWithContainersMedium,
  ] = useState(isInModal ? 1 : 2);
  const [columnsWithContainersLarge, setColumnsWithContainersLarge] = useState(
    isInModal ? 1 : 3
  );
    ...
    return {
      table: (
        <TableIcon
          ...
          width={24}
          height={24}
          className={`${name === currentView ? 'active' : ''}`}
        />
      ),
      cards: (
        <CardsIcon
  ...
  useEffect(() => {
    if (settings) {
      ...
      setColumnsInsideContainerSmall(
        settings?.columnsInsideContainerSmall || 1
      );
      setColumnsInsideContainerMedium(
        settings?.columnsInsideContainerMedium || 2
      );
      setColumnsInsideContainerLarge(
        settings?.columnsInsideContainerLarge || 3
      );

      setColumnsWithContainersSmall(settings?.columnsWithContainersSmall || 1);
      setColumnsWithContainersMedium(
        settings?.columnsWithContainersMedium ||
          (currentView === 'table' ? 1 : 2)
      );
      setColumnsWithContainersLarge(
        settings?.columnsWithContainersLarge ||
          (currentView === 'table' ? 1 : 3)
      );
    }
  }, [settings]);
        ...
        <div className="generic-data-view__views">
          <ul className="generic-data-view__view-selector">
            {viewsList.map((viewItem, index) => (
              <li key={`view${index}}`} onClick={() => toggleView(viewItem)}>
                {' '}
                <ViewIcon name={viewItem} />
              </li>
            ))}
          </ul>
        </div>
            ...
            {
              {
                table: (
                  <views.TableView
                    columnsWithContainersSmall={columnsWithContainersSmall || 1}
                    columnsWithContainersMedium={
                      columnsWithContainersMedium || 1
                    }
                    columnsWithContainersLarge={columnsWithContainersLarge || 1}
                    {...{
                      dataContainers,
                      settings,
                ...
                cards: (
                  <views.CardsView
                    ...
                    columnsWithContainersLarge={columnsWithContainersLarge || 3}
                    columnsInsideContainerSmall={
