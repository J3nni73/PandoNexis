...
export const GenericDataViewField = React.forwardRef(
  (
    ...
    const [isChecked, setIsChecked] = useState(
      props.defaultValue !== 'false' ? true : false
    );

    if (fieldSettings && fieldSettings.validationRules) {
      isRequired = fieldSettings.validationRules.find(
        (x) => x.rule === 'IsRequired'
      )
        ? true
        : false;
    ...
    if (genericButtons?.length > 0) {
      return (
        <div className="generic-data-view__btn-group" {...ref}>
          {genericButtons.map((button, index) => (
            <ButtonField
              ...
              autoFocus={
                window.currGenDW_lastClickedFieldId &&
                window.currGenDW_lastClickedEntitySystemId &&
                window.currGenDW_lastClickedEntitySystemId === entitySystemId &&
      ...
      return (
        <label className="generic-data-view__radiobutton-wrapper">
          <input
            ...
            {...ref}
          />
          <span>{fieldSettings?.placeholderText || title}</span>
        </label>
      );
      ...
      return (
        <label className="generic-data-view__checkbox-wrapper">
          <input
            ...
            key={`test${fieldId}${entitySystemId}${isChecked}`}
            {...ref}
          />
          <span>{fieldSettings?.placeholderText || title}</span>
        </label>
        ...
        default:
          return (
            <>
              ...
              <span
                className="generic-data-view__simple-text"
                title={props.defaultValue}
              >
                {props.defaultValue}
                {suffix}
