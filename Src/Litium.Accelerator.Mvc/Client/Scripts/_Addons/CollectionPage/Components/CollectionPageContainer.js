    ...
    return (
        <Fragment>
            {pageStructure.useFilters && filters && filters.length > 0 &&
                <div className="row collection-page__filters">
                    {filters.map((child, indexFilter) => {
                        return (<ul className="small-12 medium-6 large-4 columns  collection-page__filter-categories" key={`filter-${indexFilter}`}><li>
                            <div class={`dropdown-selector ${activeFilterIndex === indexFilter ? 'dropdown-selector__current' : ''}`}>
                                <div class=" button button__tertiary collection-page__filter-button" onClick={() => setActiveFilterIndex(activeFilterIndex === indexFilter ? -1 : indexFilter)}>
                                    {child.title}
