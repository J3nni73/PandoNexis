...
const CollectionPageContainer = ({
  ...
  const { pageStructure } = useSelector((state) => state.collectionPage);
  ...
  const toggleFilterValue = (event, filterValue, filterOrder) => {
    ...
    switch (filterOrder) {
      case 1:
        if (!target.checked) {
          setSelectedFilterValues1(
            selectedFilterValues1.filter(
              (e) => e.toLowerCase() !== filterValue.toLowerCase()
            )
          );
        } else if (
          !selectedFilterValues1.some(
            (e) => e.toLowerCase() === filterValue.toLowerCase()
          )
        ) {
          setSelectedFilterValues1([
            ...selectedFilterValues1,
      ...
      case 2:
        if (!target.checked) {
          setSelectedFilterValues2(
            selectedFilterValues2.filter(
              (e) => e.toLowerCase() !== filterValue.toLowerCase()
            )
          );
        } else if (
          !selectedFilterValues2.some(
            (e) => e.toLowerCase() === filterValue.toLowerCase()
          )
        ) {
          setSelectedFilterValues2([
            ...selectedFilterValues2,
            filterValue.toLowerCase(),
          ]);
        }
        break;
      case 3:
        if (!target.checked) {
          setSelectedFilterValues3(
            selectedFilterValues3.filter(
              (e) => e.toLowerCase() !== filterValue.toLowerCase()
            )
          );
        } else if (
          !selectedFilterValues3.some(
            (e) => e.toLowerCase() === filterValue.toLowerCase()
          )
        ) {
          setSelectedFilterValues3([
            ...selectedFilterValues3,
            filterValue.toLowerCase(),
          ]);
        }
        break;
  ...
  const toggleFilter = (event, filterObj) => {
    ...
    if (!target.checked) {
      setSelectedFilters(
        selectedFilters.filter((e) => e.title !== filterObj.title)
      );
  ...
  useEffect(() => {
    if (componentInit) {
      if (
        selectedFilterValues1.length < 1 &&
        selectedFilterValues2.length < 1 &&
        selectedFilterValues3.length < 1
      ) {
        setChildren(pageStructure.children);
        return;
      }
      ...
      if (selectedFilterValues1 && selectedFilterValues1.length > 0) {
        filteredChildren = filteredChildren.filter((e) =>
          selectedFilterValues1.some(
            (x) => x.toLowerCase() === e.filterValue1?.toLowerCase()
          )
        );
      }
      if (selectedFilterValues2 && selectedFilterValues2.length > 0) {
        filteredChildren = filteredChildren.filter((e) =>
          selectedFilterValues2.some(
            (x) => x.toLowerCase() === e.filterValue2?.toLowerCase()
          )
        );
      }
      if (selectedFilterValues3 && selectedFilterValues3.length > 0) {
        filteredChildren = filteredChildren.filter((e) =>
          selectedFilterValues3.some(
            (x) => x.toLowerCase() === e.filterValue3?.toLowerCase()
          )
        );
      }
  ...
  useEffect(() => {
    if (componentInit) {
      ...
      setChildren(
        pageStructure.children.filter((e) =>
          selectedFilters.some((x) =>
            x.values.some(
              (y) =>
                y.toLowerCase() === e.filterValue1?.toLowerCase() ||
                y.toLowerCase() === e.filterValue2?.toLowerCase() ||
                y.toLowerCase() === e.filterValue3?.toLowerCase()
            )
          )
        )
      );
        ...
        <div className="row collection-page__filters" ref={filterContainerRef}>
          <div className="small-12 columns">
            {filters.map((child, indexFilter) => {
              return (
                <ul
                  className="collection-page__filter-categories"
                  id={`filter-${indexFilter}`}
                  key={`filter-${indexFilter}`}
                >
                  <li>
                    <div
                      className={`dropdown-selector ${
                        activeFilterIndex === indexFilter
                          ? 'dropdown-selector__current'
                          : ''
                      }`}
                    >
                      <div
                        className=" button button__tertiary collection-page__filter-button"
                        onClick={() =>
                          setActiveFilterIndex(
                            activeFilterIndex === indexFilter ? -1 : indexFilter
                          )
                        }
                      >
                        {child.title}
                      ...
                      <ul className="collection-page__filter-values dropdown-selector__items">
                        {child.values.map((childValue, indexFilterValue) => {
                          return (
                            <li
                              className="collection-page__filter-title"
                              key={`filterValue-${indexFilterValue}`}
                            >
                              <input
                                id={`filter_${indexFilter}-${indexFilterValue}`}
                                type="checkbox"
                                onChange={(e) =>
                                  toggleFilterValue(
                                    e,
                              ...
                              />
                              <label
                                htmlFor={`filter_${indexFilter}-${indexFilterValue}`}
                              >
                                {childValue}
                              </label>
                            </li>
            ...
            {link && linkText && (
              <a
                className="collection-page__filter-button-link button"
                href={link}
                title={linkText}
              >
                {linkText}
      ...
      <div className="collection-page__data row small-up-1 medium-up-3 large-up-4">
        {children &&
          children.map((child, index) => {
            return (
              <div
                className="column collection-page__child-container"
                key={`child-${index}`}
              >
                <article className="collection-page__child">
                  <figure className="collection-page__child-image">
                    {child.image && (
                      <ConditionalLinkWrapper
                        ...
                        wrapper={(children) => (
                          <a
                            href={child.button.href}
                            target={`${
                              child.button.target ? child.button.target : ''
                            }`}
                          >
                            {children}
                          </a>
                  ...
                  >
                    <section className="collection-page__child-text">
                      <h3>{child.title}</h3>
                      <div
                        dangerouslySetInnerHTML={{ __html: child.introduction }}
                      ></div>
                    </section>
                  </ConditionalLinkWrapper>
                  {child.button && child.button.href && child.button.linkText && (
                    <a
                      className={`button button__primary collection-page__child-cta ${child.button.class}`}
                      href={child.button.href}
                      target={`${
                        child.button.target ? child.button.target : ''
                      }`}
                    >
                      {child.button.linkText}
                    </a>
                  )}
                </article>
              </div>
