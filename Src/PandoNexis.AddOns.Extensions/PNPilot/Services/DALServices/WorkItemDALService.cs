...
namespace PandoNexis.AddOns.PNPilot.Services.DALServices
{
    ...
    public class WorkItemDALService : BaseDALService
    {
                ...
                using (SqlCommand command = new SqlCommand(sql, connection))
                {
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            newItem.AssignedPersonSystemId = GetGuidValue(reader, PilotConstants.AssignedPersonSystemId);
                            newItem.ReportedByPersonSystemId = GetGuidValue(reader, PilotConstants.ReportedByPersonSystemId);
        ...
        public override bool AddOrUpdate(object workItem)
        {
            ...
            dalObject.Columns.FirstOrDefault(i => i.Name == PilotConstants.AssignedPersonSystemId).Value = item.AssignedPersonSystemId;
            dalObject.Columns.FirstOrDefault(i => i.Name == PilotConstants.ReportedByPersonSystemId).Value = item.ReportedByPersonSystemId;
